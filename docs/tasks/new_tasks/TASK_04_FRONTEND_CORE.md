...
### Frontend
- [ ] 1. Создать `.env.local` в корне `frontend/` и прописать `NEXT_PUBLIC_API_URL=http://localhost:8080`.
- [ ] 2. Создать `frontend/src/lib/types.ts`.
- [ ] 3. Создать `frontend/src/lib/api.ts` с fetch функциями.
- [ ] 4. Создать `frontend/src/lib/query-client-provider.tsx`.
- [ ] 5. Настроить `globals.css` для Tailwind CSS v4 с базовыми цветами и стилями.
- [ ] 6. Обновить `layout.tsx`: подключить шрифт Inter, обернуть в `QueryClientProvider`.
- [ ] 7. Создать `frontend/src/components/stats-cards.tsx` с анимацией при наведении.
- [ ] 8. Создать `frontend/src/components/search-filter-bar.tsx` с debounce-эффектом для поиска (200-300мс).
- [ ] 9. Собрать временный макет в `page.tsx` с фейковыми или тестовыми данными React Query.
- [ ] 10. Проверить сборку фронтенда: `cd frontend && pnpm run build`.
...
4. [ ] Синхронизируй `ROADMAP.md` — отметь выполненное `[x]` для Фазы 3 (базовая разметка и сетка).
