...
- [x] 1. Создать `backend/Dockerfile`. Проверить сборку докер-образа локально: `docker build -t lawtrack-backend ./backend`.
- [x] 2. Создать файл `README.md` в корне репозитория, заполнив его по структуре из ТЗ (раздел 7).
- [x] 3. Написать пошаговую инструкцию по деплою бэкенда на Render и фронтенда на Vercel.
- [x] 4. Запустить финальную локальную проверку: `.\verify-all.ps1` из корня проекта. Все проверки (бэкенд тесты и фронтенд сборка) должны пройти успешно.
- [x] 5. Обновить `ROADMAP.md` (выставить все галочки [x] по фазам 1-5).
- [x] 6. Обновить `docs/CONTEXT_BACKUP.md`, подведя итог всей разработки.
- [x] 7. Запустить ротацию логов: `.\rotate-backup.ps1`.
- [x] 8. Перемести файл этой задачи из `docs/tasks/new_tasks/` в `docs/tasks/temp_tasks/`.
