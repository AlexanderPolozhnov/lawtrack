...
### Backend
- [x] 1. Создать миграцию `V2__client_events.sql`.
- [x] 2. Создать Enum `ClientEventType` и Entity `ClientEvent`.
- [x] 3. Создать DTO `CreateNoteRequest` и `ClientEventResponse`.
- [x] 4. Создать `ClientEventRepository`.
- [x] 5. Обновить `ClientService` для сохранения событий `CREATED` и `STATUS_CHANGED`, а также метод `addNote` для добавления ручных заметок.
- [x] 6. Обновить `ClientController` (добавить GET `/api/clients/{id}/events` и POST `/api/clients/{id}/notes`).
- [x] 7. Скомпилировать и запустить тесты бэкенда: `cd backend && .\mvnw.cmd clean test`.

### Frontend
- [x] 8. Добавить типы и API-функции во фронтенд (`lib/types.ts`, `lib/api.ts`).
- [x] 9. Создать компонент `client-details-drawer.tsx` (используя shadcn `Sheet` или `Drawer`). Показать в нем:
  - Информацию о деле, дедлайн с индикацией просрочки.
  - Форму добавления заметки.
  - Список событий (таймлайн с точками и линиями).
- [x] 10. Обновить `client-row.tsx`, чтобы при клике на строку (кроме селектора статуса) открывался Drawer.
- [x] 11. Добавить переключатель темы в Header и настроить стили темной темы в `globals.css`.
...
4. [x] Синхронизируй `ROADMAP.md` — отметь выполненное `[x]` для Фазы 4.
