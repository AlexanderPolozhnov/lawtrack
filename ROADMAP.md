...
*Статус: Выполнено.*
- [x] Карточка клиента (`ClientDetailsDrawer`): side panel с заметками и таймлайном событий по делу (из БД `client_events`).
- [x] Индикация дедлайнов (красный индикатор, если дедлайн прошел, а дело не закрыто).
- [x] Тёмная тема с переключателем в Header.
- [x] Donut Chart (Recharts) для визуализации распределения статусов клиентов (Сокращено из MVP релиза ради фокуса на кор-фичах).

### ✅ Фаза 5 — Документация и Деплой
*Статус: Выполнено.*
- [x] Детальный README.md (запуск, стек, ссылки на деплой, лог работы).
- [x] Деплой Backend (Render.com Docker Web Service + Postgres).
- [x] Деплой Frontend (Vercel).
- [x] Финальная проверка работоспособности сценариев и валидация через `.\verify-all.ps1`.
