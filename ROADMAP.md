...
*Статус: Запланировано.*
- [ ] Базовая сетка приложения (Layout, Header с логотипом "LawTrack" и кнопкой добавления).
- [ ] API-клиент (`lib/api.ts`, `lib/types.ts`) и React Query провайдер.
- [ ] Компонент `StatsCards`: 3 карточки счетчиков по статусам с реактивным обновлением.
- [ ] Компонент `SearchFilterBar`: текстовый поиск и фильтр по статусу (влияет на счетчики по клику).
